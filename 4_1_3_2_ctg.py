...
class TangentLine(Scene):
    def construct(self):
        ...
        bisect_line = Line(start=ax.c2p(-2, 1), end=ax.c2p(2, 1), color=R_BLUE)
        bisect_point_ox = Dot(ax.c2p(0, 1), color=R_WHITE)
        ...
        color_line = Line(start=ax.c2p(0, 1), end=ax.c2p(1, 1), color=R_GREEN)
        self.play(FadeIn(color_line), run_time=1.5)

        M_text = MathTex('\\cot{\\alpha}', color=R_WHITE).move_to(
            ax.c2p(0.5, 1.3)
        )
