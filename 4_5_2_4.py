...
class Picture_4_5_2_4(Scene):
    def construct(self):
        ...
        # График арксинуса
        arcsin_graph = ax.plot(lambda x: np.arcsin(x), x_range=[-1, 1], color=R_RED)
        self.play(Write(arcsin_graph), run_time=3)
        arcsin_label = ax.get_graph_label(arcsin_graph, MathTex('y = arcsin(x)'), x_val=-1, direction=DL + 0.9 * DOWN)
        self.play(Write(arcsin_label), run_time=2)
        ...
        arcsin_graph_reversed = ax.plot(lambda y: np.sin(y), x_range=[-np.pi/2, np.pi/2], color=R_PURPLE)
        self.play(Write(arcsin_graph_reversed), run_time=3)

        dot1 = Dot(color=R_ORANGE, radius=0.08).move_to(ax @ (-1, np.arcsin(-1)))
        dot2 = Dot(color=R_ORANGE, radius=0.08).move_to(ax @ (np.pi/2, np.sin(np.pi/2)))
